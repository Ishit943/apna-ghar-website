...
router.post("/", authMiddleware, authorize("admin", "team_member"), async (req, res) => {
  try {
    const { title, location, type, price, description, sqft, beds, baths, images } = req.body;
...
router.put("/:id", authMiddleware, authorize("admin", "team_member"), async (req, res) => {
  try {
...
router.delete("/:id", authMiddleware, authorize("admin", "team_member"), async (req, res) => {
  try {
    const property = await Property.findById(req.params.id);
